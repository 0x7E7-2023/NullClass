...
> ① **平台画像不可信** —— 计划表里 `MASU`（自称青果，实为**树维 EAMS**）与 `HYNU`（走 `jsxsd`
> 路径，实为湖南强智）都标错了，后续批次要按接口路径 + 选择器结构聚类。
> （2026-09-16 批次四订正：此处原文写的是「实为强智 eams」，**厂商名错了** —— `/eams/` 是上海
> 树维（SupWisdom）的产品。当时**结构认对了、厂商认错了**，而这条错判被后来两批的文档当结论
> 抄了下去。教训见[批次四](2026-09-16-port-adapters-batch4.md)「平台名订正」一节。）
