...
> 最后更新：2026-09-06，M3/M4/M5 完成（本地 HEAD `307bfa8`），CI 待推送验证。
...
- **版本约定**：M2=0.1.0、M3=0.2.0、M4=0.3.0（首发版）、M5=0.4.0（当前 versionName）
...
1. **AGP 9.3 内置 Kotlin ≠ 内置 Compose 编译器**：Compose 模块必须再 apply `org.jetbrains.kotlin.plugin.compose`（version catalog 里 `kotlin-compose`）。**Glance 模块也是 Compose 编译目标**（M3 实测：不 apply 会编译期内部错误/不可用）
...
4. Glance 1.2：`SizeMode.Responsive` 收 `Set<DpSize>`；day/night `ColorProvider` 在 `androidx.glance.color` 包（参数是 Compose `Color`）；`material-icons-core` **不随 material3 传递**，用到 `Icons.Default.*` 的模块要显式依赖；**LazyColumn 在 `androidx.glance.appwidget.lazy` 包**；`LocalSize` 在 `androidx.glance`（核心包）；`clickable` 是 `androidx.glance.action.clickable`；`:app` 直接用 `updateAll` 扩展需显式依赖 glance-appwidget
...
9. Room suspend DAO 与 kotlinx.serialization 都有 `intOrNull`：JsonElement 的扩展属性直接用，别自己定义同名函数
10. **Room JournalMode 切到 TRUNCATE**（M3 落地，见数据架构）：WAL 在雷电模拟器上跨连接失效通知不工作且 install 丢未 checkpoint 数据；本应用低频写入不需要 WAL 并发。真机 WAL 没问题，但保持 TRUNCATE 无害且省去模拟器坑
...
- **Room JournalMode.TRUNCATE**（M3 起，见踩坑 10）
...
- **共享格式**：`.nullclass` 文件 = WebDAV snapshot.json = `ScheduleDocument`（formatVersion 2，定义在 `:importer`）；`:sync` 依赖 `:importer`（反向不行）。导入 = `SyncEngine.merge` 一次单向同步，LWW/墓碑/period_times 整组取新全部复用
- **本地库 ↔ 文档**：`SnapshotCodec`（dump/apply/countAdopted）是同步与导入导出共用入口；`SyncManager` 只剩编排
- **导入管线**：文件/二维码/WakeUp/教务四条通道 → `TransferViewModel.parseRaw` → `ImportPreview`（预览）→ `confirmMerge`
- **WakeUp**：逐行内容特征识别（不依赖行号）；`.wakeup_schedule` 行结构见 `docs/samples/wakeup_schedule_sample.md`；解析器在 `:importer/wakeup/WakeUpParser.kt`，fixture 驱动单测
- **教务框架**：`:importer/jw/`（JwAdapter 接口 + 注册表 + 示例适配器，纯 JVM）+ `:feature:settings/jw/JwImportActivity`（WebView 宿主）；凭证红线：登录全程用户手工完成。**真实学校适配器待用户指认学校后开发**
- **提醒/小组件**：`ReminderPlanner`（core:model 纯函数）+ 每条提醒一个 uniqueWork；小组件 EntryPoint 取数；推送（WidgetAutoUpdater）+ 每日 Worker 兜底双轨
- **WorkManager 按需初始化**：`Configuration.Provider` + `HiltWorkerFactory`，manifest 移除了默认初始化器
...
- [M5 详细文档](2026-09-06-m5-detailed.md)——教务导入框架（已完成；真实学校适配器待用户指认）
- [M3 实现文档](2026-09-06-m3.md)——小组件接数据、课前提醒、自动同步（已完成）
- [M4 实现文档](2026-09-06-m4.md)——导入导出、二维码分享、WakeUp 适配、首发 Release（已完成；tag 待 secrets 配置）
- [M5 概要文档](2026-09-06-m5.md)——教务系统 WebView 导入原始构想
